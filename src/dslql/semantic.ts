...
export type DslqlSemanticEmbedder = (
  texts: string[],
) => Promise<DslqlSemanticEmbeddingResult | undefined>;

export type DslqlSemanticTextSelector = (
  value: DslqlObject,
) => string | undefined;
...
export interface SemanticDocumentDslqlRuntimeOptions
  extends DocumentDslqlRuntimeOptions, SemanticDslqlRuntimeOptions {}
...
function semanticText(
  value: DslqlObject,
  visited = new WeakSet<DslqlObject>(),
): string | undefined {
...
function collectSemanticCandidates(
  ...
  function collect(value: DslqlValue): void {
    ...
    if (text) candidates.push({ value: object, text });
...
function targetFromOperand(
  ...
  throw new DslqlSemanticError(
    "Semantic operands must be the current object '.', an @ID reference, or a non-empty string literal; dynamic construction is not supported",
    operand.range,
  );
}
...
function validateThreshold(
  ...
  if (!threshold) return;
  if (
    threshold.kind !== "literal" ||
    typeof threshold.value !== "number" ||
    threshold.value < 0 ||
    threshold.value > 1
  ) {
    throw new DslqlSemanticError(
      `${expression.name}() threshold must be a number literal from 0 through 1`,
      threshold.range,
    );
  }
}
...
function validateSemanticCall(
  ...
  if (expression.name === "similarity") {
    ...
    if (operands[0]?.kind === "path") {
      throw new DslqlSemanticError(
        "nearest_to() target must be an @ID reference or a non-empty string literal",
        operands[0].range,
      );
    }
  }
...
function collectSemanticTargets(expression: DslqlExpression): SemanticTarget[] {
  const targets = new Map<string, SemanticTarget>();
  visitDslqlAst(expression, (node) => {
    if (node.kind !== "call" || !SEMANTIC_FUNCTIONS.has(node.name)) return;
...
function validateOnDemandEmbeddingBudget(
  targets: readonly SemanticTarget[],
  options: SemanticDslqlRuntimeOptions,
): void {
  const limit =
    options.maxOnDemandEmbeddings ?? DEFAULT_DSLQL_ON_DEMAND_EMBEDDING_LIMIT;
  if (!Number.isSafeInteger(limit) || limit < 0) {
    throw new DslqlSemanticError(
      "maxOnDemandEmbeddings must be a non-negative safe integer",
    );
  }
  ...
  if (demanded > limit) {
    throw new DslqlSemanticError(
      `Semantic query requires ${demanded} distinct on-demand embeddings; configured limit is ${limit}`,
    );
  }
}

export function usesSemanticDslql(
  ...
  visitDslqlAst(ast, (node) => {
    if (node.kind === "call" && SEMANTIC_FUNCTIONS.has(node.name)) found = true;
  });
  return found;
}

function needsObjectEmbeddings(expression: DslqlExpression): boolean {
  let needed = false;
  visitDslqlAst(expression, (node) => {
    if (node.kind !== "call" || !SEMANTIC_FUNCTIONS.has(node.name)) return;
...
function validateEmbeddingResult(
  result: DslqlSemanticEmbeddingResult,
  ...
  if (invalid) {
    throw new DslqlSemanticUnavailableError(
...
function vectorForStaticOperand(
  operand: DslqlExpression,
...
function vectorForObject(
  value: DslqlValue,
  vectorByValue: WeakMap<DslqlObject, number[]>,
): number[] | undefined {
  const object = asObject(value);
...
function resolveSemanticOperand(
  operand: DslqlExpression,
  item: DslqlValue,
  ...
  if (staticVector) return staticVector;

  const values = context.evaluate(operand, [item]);
  if (values.length !== 1) {
    throw new DslqlSemanticError(
      `Semantic operand must produce exactly one object; received ${values.length}`,
      operand.range,
    );
  }
  ...
  if (!vector) {
    throw new DslqlSemanticError(
      "Semantic operand must be an embedded object; only string literals may be embedded on demand",
      operand.range,
    );
  }
  return vector;
...
function createSimilarityFunction(
  vectorByValue: WeakMap<DslqlObject, number[]>,
  targetVectors: Map<string, number[]>,
): DslqlFunction {
  return (context) =>
    context.input.map((item) => {
      const left = resolveSemanticOperand(
        context.arguments[0]!,
        item,
        context,
        vectorByValue,
      ...
      const right = resolveSemanticOperand(
        context.arguments[1]!,
        item,
        context,
        vectorByValue,
...
function createSimilarToPredicateFunction(
  vectorByValue: WeakMap<DslqlObject, number[]>,
  targetVectors: Map<string, number[]>,
): DslqlFunction {
  ...
  return (context) => {
    ...
    const threshold =
      thresholdExpression.kind === "literal" &&
...
function createNearestToFunction(
  vectorByValue: WeakMap<DslqlObject, number[]>,
  targetVectors: Map<string, number[]>,
  provider: string,
  model: string,
  ...
  return (context) => {
    const call: DslqlCallExpression = {
      kind: "call",
      name: "nearest_to",
    ...
    if (!targetVector) {
      throw new DslqlSemanticError(
        `Semantic target '${target.referenceId ?? target.text ?? target.key}' was not prepared`,
        target.range,
      );
    }
    ...
    return context.input
      .map((value, position) => {
        ...
        if (!node || !vector) {
          throw new DslqlSemanticError(
            "nearest_to() input must be a semantic node from the prepared runtime",
          );
        }
...
async function prepareEmbeddings(
  texts: string[],
  options: SemanticDslqlRuntimeOptions,
): Promise<DslqlSemanticEmbeddingResult> {
  let result: DslqlSemanticEmbeddingResult | undefined;
  try {
    ...
    throw new DslqlSemanticUnavailableError(
      `Embedding preparation failed: ${message}`,
    );
  }
  if (!result) {
    throw new DslqlSemanticUnavailableError(
      "Embedding provider is disabled; semantic DSLQL requires an embedding provider",
    );
  }
...
export async function createSemanticDslqlRuntime(
  runtime: DslqlRuntime,
  expression: string | DslqlExpression,
  options: SemanticDslqlRuntimeOptions = {},
): Promise<DslqlRuntime> {
  const ast =
    typeof expression === "string"
      ? parseDslqlExpression(expression)
      : expression;
  ...
  if (targets.length === 0 && !objectEmbeddingsNeeded) return runtime;
  ...
  const vectorByText = new Map(
    uniqueTexts.map(
      (text, index) => [text, result.embeddings[index]!] as const,
    ),
  );
  const vectorByValue = new WeakMap<DslqlObject, number[]>();
  candidates.forEach((candidate) => {
    const vector = vectorByText.get(candidate.text);
    if (vector) vectorByValue.set(candidate.value, vector);
  });
...
export async function createSemanticDocumentDslqlRuntime(
  documentAst: DocumentAst,
  expression: string | DslqlExpression,
  options: SemanticDocumentDslqlRuntimeOptions = {},
): Promise<DslqlRuntime> {
  return createSemanticDslqlRuntime(
    createDocumentDslqlRuntime(documentAst, options),
    expression,
    options,
  );
}

export async function evaluateSemanticDslqlExpression(
  expression: string | DslqlExpression,
  runtime: DslqlRuntime,
  options: SemanticDslqlRuntimeOptions = {},
): Promise<DslqlValue[]> {
...
export async function evaluateSemanticDocumentDslqlExpression(
  expression: string | DslqlExpression,
  documentAst: DocumentAst,
  options: SemanticDocumentDslqlRuntimeOptions = {},
): Promise<DslqlValue[]> {
  return evaluateSemanticDslqlExpression(
    expression,
    createDocumentDslqlRuntime(documentAst, options),
    options,
  );
