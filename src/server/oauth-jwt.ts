...
export type LlmthinkOAuthAccountResolver = (
...
function parseExactHttpsUrl(value: string, field: string): URL {
  let url: URL;
  try {
    url = new URL(value);
  } catch {
...
function tokenScopes(payload: JWTPayload): readonly string[] {
...
export function createLlmthinkRemoteJwks(
  options: LlmthinkRemoteJwksOptions,
): JWTVerifyGetKey {
...
export function createLlmthinkJwtIdentityVerifier(
  ...
  const allowedAuthorizedParties = boundedUniqueStrings(
    options.allowedAuthorizedParties,
    "OAuth authorized parties",
  );
  const allowedScopes = boundedUniqueStrings(
    options.allowedTokenScopes,
    "OAuth allowed token scopes",
  );
  const requiredScopes = boundedUniqueStrings(
    options.requiredTokenScopes,
    "OAuth required token scopes",
  );
  if (
    algorithms.length === 0 ||
    algorithms.length > 8 ||
    ...
    throw new Error(
      "OAuth JWT algorithms must be a bounded asymmetric allowlist",
    );
  }
  if (
    requiredScopes &&
  ...
  const acceptedIdentity = (payload: JWTPayload) => {
    const subjectId = payload.sub;
    if (!subjectId) throw new Error("OAuth subject claim is required");
