# Impact-Aware RCA Profile R1 — 要求候補 R3 日本語全文訳

本書は `rca-profile-r1-r3.md` のレビュー支援用日本語訳であり、正本ではない。要求のauthorityは
英語正本の確定bytesにある。
...
### 2.1 R2 review findingの扱い

R3は、オーナーが`RCA REVISE`を選択した後、未受入のRCA要求R2候補をsupersedeする。完了済みR2独立レビューの
要求model findingだけを取り込む。

- failedまたはinconclusive verification向けに表現可能なsuccessor-action relationを定義する。
- impact obligationを、定義済みのimpact-to-targetとcurrent-assessment構造だけで記述する。
- 各phenomenonを適用対象impact scopeへbindingする。
- 全relation endpoint categoryを正確なnode-kind集合へ展開する。

R2レビュー報告は本revisionのevidenceであり、規範要求textではない。R2 review statusは引き継がず、R3は
Step 1から再開する。
...
7. `completeness=complete`が主張するのは、authorが宣言したuniverseの全memberにassessmentがあること
   だけであり、選択universeが現実世界を網羅することは証明しない。
8. `partial`と`unknown`のscope completenessはraw reportへ残し、caseの構造的完了を禁止する。
...
8. Failedまたはinconclusive verificationではeffectivenessを未解決のままにし、`pending`からincoming
   `tracks` relationを持つか、section 4.2で許可されるsuccessor action kindへoutgoing
   `requires_followup` relationを持たなければならない。
...
| `RCA-R1-012` | failed/inconclusive verificationにincoming pending trackerとoutgoing `requires_followup` successor actionの両方がない                                          | warning | error  |
...
5. realized/credible impactが区別され、明示targetへlinkされる。
6. root、contributing、trigger、escape causeが構造的に区別される。
7. corrective action、containment、recovery、recurrence prevention、verificationが区別される。
8. 12の安定ruleが受入済みGeneric V2 closed constraint operatorだけを使う。
9. 6 fixtureすべてが全必須surfaceで正確な期待finding/spanを生成する。
10. 完全fixtureにはblocking R1 findingがなく、各不完全fixtureは無関係なrule noiseに埋もれず意図ruleで
    失敗する。
11. Failed/inconclusive verificationがfollow-up obligationを満たせるのは、incoming pending trackerまたは
    exact allowed action kindへのoutgoing `requires_followup` relationを持つ場合だけとする。
12. Profile追加が無関係なV1/V2 documentのfinding、AST、help default、storage、behaviorを暗黙変更しない。
13. 悪意あるtarget reference/profile propertyがI/Oまたはcode executionを引き起こせない。
14. MigrationがproseからRCA semanticsを推定せず、未解決roleをすべてlocation付きで報告する。
15. Audit PASSとcase `settled`をstructural conformanceとしてのみ提示する。
16. 全canonical RCA topic、安定rule detail、対応alias、fixtureが、profile固有parser/dispatch codeなしで、
    全必須surfaceのV2 profile indexから到達できる。
17. 全help exampleが表示された`rca-impact@1.0.0` referenceでparse/auditでき、宣言済み安定rule IDだけを
    生成し、invalid routeは決定論的なoffline recoveryを提示する。
18. 修飾なしV1 helpとV1 RCA類似guidanceが変更されず、profile選択またはmigrationを主張しない。
19. Implementation change setにrelease、publication、deployment、Issue mutation、external action、Generic V2
...
- R2のsuccessor-action、credible-impact、scope-association、endpoint-kind findingを、parser syntaxまたは
  executable profile behaviorを追加せず明示的に解消した。
