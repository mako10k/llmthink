# Impact-Aware RCA Profile R1 — Requirement Candidate R3
...
### 2.1 R2 review disposition

R3 supersedes the unaccepted RCA requirement R2 candidate after the owner selected `RCA REVISE`.
It incorporates only the requirement-model findings from the completed R2 independent review:

- define a representable successor-action relation for failed or inconclusive verification;
- state the impact obligation entirely through defined impact-to-target and current-assessment
  structure;
- bind every phenomenon to its applicable impact scope; and
- expand every relation endpoint category to an exact node-kind set.

The R2 review report is evidence for this revision, not normative requirement text. R2 review status
does not carry forward; R3 starts again at Step 1.
...
7. `completeness=complete` asserts only that every member of the author-declared universe has an
   assessment. It does not prove that the chosen universe covers the real world.
8. `partial` and `unknown` scope completeness MUST remain visible in the raw report and prevent a case
...
8. A failed or inconclusive verification MUST keep effectiveness unresolved and MUST either be the
   target of an incoming `tracks` relation from `pending` or have an outgoing `requires_followup`
   relation to a successor action kind allowed by section 4.2.
...
| `RCA-R1-012` | failed/inconclusive verification lacks incoming pending tracker and outgoing `requires_followup` successor action                                                                                              | warning | error  |
...
4. every phenomenon has exactly one same-case scope association, every impact has a phenomenon
   ...
   assessment;
5. realized and credible impacts are distinct and linked to explicit targets;
6. root, contributing, trigger, and escape causes remain structurally distinct;
7. corrective action, containment, recovery, recurrence prevention, and verification remain distinct;
8. all twelve stable rules use only accepted closed Generic V2 constraint operators;
9. all six fixtures produce their exact expected findings and spans across every required surface;
10. the complete fixture has no blocking R1 finding, while each incomplete fixture fails for its
    intended rule without unrelated rule noise obscuring the result;
11. failed or inconclusive verification can satisfy its follow-up obligation only through an incoming
    pending tracker or an outgoing `requires_followup` relation to an exact allowed action kind;
12. adding the profile causes no implicit finding, AST, help-default, storage, or behavior change for
    unrelated V1 or V2 documents;
13. malicious target references and profile properties cannot trigger I/O or code execution;
14. migration never infers RCA semantics from prose and reports every unresolved role with location;
15. audit PASS and case `settled` are presented only as structural conformance;
16. every canonical RCA topic, stable-rule detail, supported alias, and fixture is reachable from
    the V2 profile index on every required surface without profile-specific parser or dispatch code;
17. every help example parses and audits under the displayed `rca-impact@1.0.0` reference and
    produces only its declared stable rule IDs, while invalid routes offer deterministic offline
    recovery;
18. unqualified V1 help and V1 RCA-like guidance remain unchanged and never claim profile selection
    or migration; and
19. no release, publication, deployment, Issue mutation, external action, Generic V2 acceptance, or
...
- The R2 successor-action, credible-impact, scope-association, and endpoint-kind findings are resolved
  explicitly without adding parser syntax or executable profile behavior.
