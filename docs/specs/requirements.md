...
- 発想支援のように、発散、収束、クラスタ化、ラベル付け、結論化の流れを既存 role の組み合わせで記述できる
- 課題解決や問題解決のように、問題、前提、根拠、判断、保留を最小要素として追跡できる
...
- 類似した過去の思考記述を検索する
- 意味近傍や関係グラフを補助的に参照する
- 設計レビュー、比較検討、計画整理、振り返りなどのその他ユースケースを、syntax 追加ではなく examples と guidance で段階的に案内する

### 5.3 ユースケースプロファイル方針

- ユースケース差分は、可能な限り新しい core syntax ではなく既存 role の使い分けで吸収する
- 発想支援、課題解決、その他ユースケースには、それぞれ最小要素の profile と representative example を用意する
- alias は parser keyword ではなく docs や help 上の説明語として扱う
- usecase ごとの guidance は examples、query templates、preview 表示説明で補助し、AST と audit の role 名は固定する
