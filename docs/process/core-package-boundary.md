...
依存方向はroot applicationから`@llmthink/core`と`@llmthink/server`へ、`@llmthink/server`から`@llmthink/core`と`@llmthink/contracts`への一方向とする。Coreとserver sourceはroot `src`、外部plugin、vscode-extensionを参照しない。pluginは固定MCP contract snapshotだけに依存し、本repositoryのsourceをimportしない。
