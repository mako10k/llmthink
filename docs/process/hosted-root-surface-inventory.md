...
| ---------------------------------------------------------------------------------- | ----------------------------- | ---------------------------------------------------------------- |
| `DeleteThoughtCommand`, `ThoughtDeletionReceipt`                                   | shared Hosted contract        | canonical v1のdelete schemaと同じ意味を持つためContracts正本候補 |
...
## Meaningful next boundary

配布方式を決める前に実施可能で、単独の意味を持つ変更は、共有Hosted APIの正本を
Contractsへ移し、Serverがその正本を使用することである。rootのimplementation compatibility
facadeとbinはその段階では維持する。最終的なroot runtime dependency除去は、公開互換surfaceの
移行先がownerに承認されるまで未完了とする。
