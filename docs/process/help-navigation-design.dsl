...
domain HelpNavigation:
  description "DSL 文法ヘルプを索引付きの案内システムとして再構成し、query と DSLQL の導線を強化する設計を整理する"
...
problem P4:
  "query の書き方は field access、pipe、select、related_decisions など複数の概念をまたぐため、逆引きと関連索引がないと把握しづらい"
...
step S3:
  evidence EV1:
    "現行 guidance は top-level blocks と基本規則を 1 つの静的テキストへ並べており、query や DSLQL の詳細へ段階的に降りる見出し構造を持たない"
...
step S5:
  evidence EV3:
    "query は .problems[]、select(.id == \"P1\")、related_decisions、projection など複数要素の組み合わせで成立するため、構文一覧だけでは書き始めにくい"
...
step S7:
  decision D1 based_on PR1, PR2, EV1, EV4:
    "help は単一文字列ではなく、index、reference_quick、reference_detail、usecase_index、related_index の複数セクションを持つ構造化リソースとして管理する"
...
step S9:
  decision D3 based_on PR2, EV4, D1, D2:
    "help API は topic、subtopic、audience、channel、detail_level を受け取り、1 回の応答は index 1 件または関連セクション 2 から 4 件までに制限する"

step S10:
  decision D4 based_on D1, D3:
    "CLI は `llmthink dsl help [topic] [subtopic]`、MCP は `dsl action=help topic=... subtopic=... detail=...`、VSIX tool は同じ論理引数を受ける形へ寄せ、全チャネルで同じ help graph を辿れるようにする"

step S11:
  decision D5 based_on EV3, D2, D3:
    "parse error 時は全体文法を再掲する代わりに、原因に対応する quick reference と related index を返し、必要時だけ detail reference や query examples へ追跡できる導線を付ける"

step S12:
  decision D6 based_on D2, D3, D5:
    "use case reverse index は `problem から decision を辿りたい`、`根拠のない decision を見つけたい`、`pending を含む step を探したい` のような目的文から query テンプレートと関連 reference を返す"
