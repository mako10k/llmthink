...
const PARSE_ERROR_HELP_RULES = [
    ...
    {
        matches: (message) => startsWithAny(message, ["Invalid domain declaration", "Domain description is required"]),
        help: {
    ...
    {
        matches: (message) => startsWithAny(message, ["Invalid problem declaration", "Problem text is required"]),
        help: {
    ...
    {
        matches: (message) => startsWithAny(message, ["Invalid premise declaration", "premise text is required"]),
        help: {
    ...
    {
        matches: (message) => startsWithAny(message, ["Invalid evidence declaration", "evidence text is required"]),
        help: {
    ...
    {
        matches: (message) => startsWithAny(message, ["Invalid pending declaration", "pending text is required"]),
        help: {
    ...
    {
        matches: (message) => startsWithAny(message, ["Invalid viewpoint declaration", "Viewpoint axis is required"]),
        help: {
    ...
    {
        matches: (message) => startsWithAny(message, ["Invalid partition declaration", "Invalid partition member"]),
        help: {
    ...
    {
        matches: (message) => startsWithAny(message, ["Invalid annotation declaration", "Annotation text is required"]),
        help: {
            rationale: "annotation は閉じた kind 集合を持ち、header 行の次に quoted text を置く。kind 名が未知だったり本文が無い場合は annotation 自体の構文を見直す。",
            expectedSyntax: [
                "problem P1:",
                '  "監査したい問題文"',
                "  annotation rationale:",
    ...
    {
        matches: (message) => startsWithAny(message, ["Invalid decision declaration", "Decision text is required"]),
        help: {
    ...
    {
        matches: (message) => startsWithAny(message, ["Invalid query declaration", "Query expression is required"]),
        help: {
