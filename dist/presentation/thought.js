...
function collectThoughtSupportPairs(text) {
    if (!text) {
        return [];
    }
    const document = parseDocument(text);
    const statementRoles = new Map(document.steps.map((step) => [step.statement.id, step.statement.role]));
...
function buildSemanticAuditOverview(snapshot) {
    ...
    const unreviewedPairs = supportPairs.filter((pair) => !reviewedPairs.has(`${pair.decisionId}::${pair.supportId}`));
    return { entries, unreviewedPairs };
}
function verdictCount(entries, verdict) {
    return entries.filter((entry) => entry.verdict === verdict).length;
}
export function formatPersistedThoughtAudit(persisted) {
    return ([
...
export function formatThoughtSummary(snapshot) {
...
export function formatThoughtHistory(history) {
    if (history.length === 0) {
...
export function formatThoughtList(records) {
    if (records.length === 0) {
        return "No persisted thoughts.\n";
    }
    return (records
        .map((thought) => `- ${thought.id} [${thought.status}] updated_at=${thought.updated_at}`)
        .join("\n") + "\n");
}
export function formatThoughtReflections(reflections) {
    if (reflections.length === 0) {
        return "No reflections yet.\n";
    }
    return (reflections
        .map((reflection) => `- ${reflection.at} [${reflection.kind}] ${reflection.text}`)
        .join("\n") + "\n");
}
export function formatThoughtSemanticAuditSummary(snapshot) {
...
export function formatThoughtSemanticAuditPairs(snapshot) {
    const overview = buildSemanticAuditOverview(snapshot);
    if (!snapshot.semanticAuditText) {
        return "No semantic audit yet.\n";
    }
...
export function formatThoughtSearchResults(results) {
    if (results.length === 0) {
