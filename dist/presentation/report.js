...
function sortIssuesBySeverity(issues) {
    return issues
        ...
        .sort((left, right) => {
        const severityDiff = SEVERITY_PRIORITY[left.issue.severity] -
...
export function limitAuditReport(report, options = {}) {
    ...
    const limitedIssues = maxIssues > 0 ? [...visibleIssues, overflowIssue] : [overflowIssue];
    return {
        ...report,
...
function appendIssueDetails(lines, issue) {
    appendOptionalLine(lines, "rationale", issue.rationale);
    appendOptionalLine(lines, "suggestion", issue.suggestion);
    const expectedSyntax = typeof issue.metadata?.expected_syntax === "string"
    ...
    const syntaxHelp = typeof issue.metadata?.syntax_help === "string"
        ? issue.metadata.syntax_help
        : undefined;
    if (syntaxHelp) {
        lines.push(`  syntax_help: ${syntaxHelp}`);
    }
    const syntaxGuidance = typeof issue.metadata?.syntax_guidance === "string"
...
export function formatAuditReportText(report, options = {}) {
    ...
    if (limitedReport.query_results.length > 0) {
        ...
        for (const queryResult of limitedReport.query_results) {
            ...
            const originalQueryResult = report.query_results.find((candidate) => candidate.query_id === queryResult.query_id);
            const omittedItems = (originalQueryResult?.items.length ?? queryResult.items.length) -
...
function issueDetailsHtml(issue) {
    const parts = [];
    if (issue.rationale) {
...
export function formatAuditReportHtml(report, options = {}) {
    const limitedReport = limitAuditReport(report, options);
    ...
    const queryRows = limitedReport.query_results
        ...
        const renderedItems = queryResult.items
            ...
            .join(", ");
        const omittedSuffix = omittedItems > 0 ? `, ... ${omittedItems} more` : "";
        return `
