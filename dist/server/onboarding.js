...
function page(terms, privacy, nonce, mode) {
    const action = mode === "reconsent" ? "再同意する" : "同意して試験利用を開始する";
    return `<!doctype html>
...
function cleanSessions(runtime) {
...
async function authenticateOnboarding(runtime, request, response) {
    try {
        ...
        sendHtml(response, 401, resultPage("認証が必要です", "ログインしてから再度お試しください。"));
        return undefined;
    }
}
function issueSession(runtime, principal, terms, privacy) {
...
function handleOnboardingGet(runtime, principal, response) {
    ...
    if (runtime.sessions.size >= runtime.maxSessions) {
        sendHtml(response, 503, resultPage("現在利用できません", "しばらくしてから再度お試しください。"));
        return;
    }
    const state = runtime.options.store.onboardingAccountState(principal.identity);
    if (state === "active") {
        sendHtml(response, 200, resultPage("同意済みです", "llmthinkを利用できます。"));
        return;
    }
    if (state === "unavailable") {
        sendHtml(response, 403, resultPage("利用できません", "問い合わせ先へご連絡ください。"));
        return;
    }
    const terms = runtime.options.store.activeTermsArtifact(runtime.options.termsId);
    const privacy = runtime.options.store.activeTermsArtifact(runtime.options.privacyNoticeId, "privacy_notice");
    const issued = issueSession(runtime, principal, terms, privacy);
    response.setHeader("set-cookie", `${SESSION_COOKIE}=${issued.csrf}; Path=/onboarding; Max-Age=${Math.floor(runtime.ttl / 1000)}; Secure; HttpOnly; SameSite=Strict`);
    sendHtml(response, 200, page(terms, privacy, issued.nonce, state === "reconsent_required" ? "reconsent" : "agree"));
}
function sessionMatches(runtime, request, principal, form, session) {
    if (!session)
        return false;
    const expected = [
    ...
    return (session.expiresAt > runtime.now() &&
...
function artifactsMatch(terms, privacy, session) {
    return [
...
async function handleOnboardingPost(runtime, request, principal, response) {
    ...
    try {
        ...
        privacy = runtime.options.store.activeTermsArtifact(session.privacyNoticeId, "privacy_notice");
    }
    catch {
        sendHtml(response, 409, resultPage("規約が更新されました", "内容を再確認してください。"));
        return;
    }
    ...
    if (state === "reconsent_required") {
        runtime.options.store.recordReconsent(principal.identity, session.termsId, TRIAL_AGREEMENT_ACTION_VERSION);
        sendHtml(response, 200, resultPage("再同意が完了しました", "llmthinkを再び利用できます。"));
        return;
    }
