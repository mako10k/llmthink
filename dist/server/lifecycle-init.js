...
function readArtifact(manifestDirectory, artifact) {
    const content = readFileSync(resolve(manifestDirectory, artifact.content_path), "utf8");
...
export function initializeLifecycleDatabase(databasePath, manifestPath) {
    ...
    try {
        ...
        const activePrivacy = store.activeTermsArtifact(manifest.privacy_notice.id, "privacy_notice");
...
function argumentsFrom(argv) {
    ...
    if (argv.length !== 4 ||
        ...
        throw new Error("Usage: llmthink-lifecycle-init --database /absolute/path --manifest /absolute/path");
    }
    return {
...
if (process.argv[1]?.endsWith("lifecycle-init.js")) {
    ...
    process.stdout.write(`${JSON.stringify(initializeLifecycleDatabase(args.databasePath, args.manifestPath))}\n`);
}
//# sourceMappingURL=lifecycle-init.js.map