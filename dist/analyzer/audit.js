...
function groupComparisonsByScope(comparisons) {
...
function comparisonPairKey(comparison) {
    return [comparison.leftDecisionId, comparison.rightDecisionId]
        .sort()
        .join("::");
}
function preferenceDirection(comparison) {
...
function buildPreferenceModel(comparisons) {
    const edges = new Map();
...
function addPreferenceConflictIssues(issues, comparisons, incomparablePairs) {
    for (const comparison of comparisons) {
        if (!preferenceDirection(comparison.statement) ||
...
function hasPreferencePath(edges, origin, current, seen) {
...
function addPreferenceCycleIssues(issues, comparisons, edges) {
    for (const comparison of comparisons) {
...
function indexIncomingCounterexamples(comparisons) {
...
function addStatusValueIssues(issues, target) {
...
function addCounterexampleDirectionIssues(issues, comparisons, decisionById) {
    for (const comparison of comparisons) {
        if (comparison.statement.relation !== "counterexample_to") {
            continue;
        }
...
function addNegatedDecisionSupportIssues(issues, decisions, incomingCounterexamples) {
...
function addStatusAnnotationIssues(issues, document, decisions, comparisons) {
    ...
    addNegatedDecisionSupportIssues(issues, decisions, indexIncomingCounterexamples(comparisons));
}
