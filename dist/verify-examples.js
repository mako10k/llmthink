...
import { auditDslFile, getDslExample } from "@llmthink/core";
const exampleCases = [
    {
...
async function main() {
    ...
    for (const exampleCase of exampleCases) {
        ...
        const actual = normalize(await auditDslFile(resolve(process.cwd(), input.path), {
            embeddings: { provider: "none" },
        }));
        const expected = JSON.parse(readFileSync(resolve(process.cwd(), expectedOutput.path), "utf8"));
...
await main();
//# sourceMappingURL=verify-examples.js.map