...
function requestDigestProperty(body: RpcBody, name: string) {
  return body.result.tools.find((tool) => tool.name === name)?.inputSchema
    .properties?.request_digest;
}

function helpRequestDigest(body: RpcBody, name: string) {
  return body.result.structuredContent.tools?.[name]?.request_digest;
}

function runtimeEffect(annotations: Record<string, boolean>) {
  if (annotations.readOnlyHint) return "read_only";
...
test("hosted MCP publishes the admitted contract tools with effect annotations", async (t) => {
  ...
  const expected = hostedMcpProducerSurface().admitted.map((tool) => ({
    ...tool,
  ...
  assert.deepEqual(
    tools.map((tool) => ({
      name: tool.name,
      effect: runtimeEffect(tool.annotations),
...
test("write tools expose actionable request_digest contracts", async (t) => {
  ...
  for (const name of [
    ...
    const property = requestDigestProperty(listed.body, name);
  ...
  const digest = helpRequestDigest(help.body, "create_thought_draft");
...
test("request_digest validation explains malformed values consistently", async (t) => {
  ...
  for (const name of [
    ...
    for (const request_digest of ["unprefixed", "sha256:xyz"]) {
      ...
      } else if (name === "delete_thought") {
        args = { thought_id: "t", expected_revision: 0 };
      }
