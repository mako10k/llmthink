...
import type {
  LlmthinkExternalAccountIdentity,
  LlmthinkLifecycleAccountResolver,
...
export function assertSqliteLifecycleNodeVersion(
...
export class SqliteLifecycleStore {
  ...
  onboardingAccountState(
    identityInput: LlmthinkExternalAccountIdentity,
  ): OnboardingAccountState {
  ...
  requestRecovery(
    ...
    proposedIdentityInput: LlmthinkExternalAccountIdentity,
  ): RecoveryRequest {
  ...
  accountResolver(): LlmthinkLifecycleAccountResolver {
    return async (identity) => {
      ...
      try {
        ...
        if (!row) throw new Error("External account mapping is unavailable");
  ...
  #configure(busyTimeoutMs: number): void {
    ...
    const busyTimeout = this.#db.prepare("PRAGMA busy_timeout").get() as Row;
    if (
  ...
  #identity(identity: LlmthinkExternalAccountIdentity) {
    return Object.freeze({
      ...
      organizationKey:
        identity.organizationId === undefined
          ...
          : boundedIdentity(
              ...
              "external identity organization",
            ),
    });
  }
