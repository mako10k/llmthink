...
import type { LlmthinkExternalAccountIdentity, LlmthinkLifecycleAccountResolver } from "./lifecycle-identity.js";
...
export interface ProvisionTrialAccountInput {
    readonly identity: LlmthinkExternalAccountIdentity;
    readonly termsId: string;
    readonly scopePolicyId: string;
...
export declare class SqliteLifecycleStore {
    ...
    onboardingAccountState(identityInput: LlmthinkExternalAccountIdentity): OnboardingAccountState;
    recordReconsent(identityInput: LlmthinkExternalAccountIdentity, termsId: string, actionVersion: typeof ACTION_VERSION): string;
    transitionAccount(identityInput: LlmthinkExternalAccountIdentity, toState: OperatorAccountState, reasonCode: string): OperatorAccountState;
    requestRecovery(recoveryCredential: string, proposedIdentityInput: LlmthinkExternalAccountIdentity): RecoveryRequest;
    ...
    recordArchive(identityInput: LlmthinkExternalAccountIdentity, input: {
        readonly contentSha256: string;
        readonly byteLength: number;
        readonly itemCount: number;
    }): ArchiveReceipt;
    archiveContext(identityInput: LlmthinkExternalAccountIdentity): ArchiveAccessContext;
    ...
    accountResolver(): LlmthinkLifecycleAccountResolver;
