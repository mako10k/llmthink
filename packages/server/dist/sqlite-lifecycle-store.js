...
import { closeSync, constants, existsSync, fchmodSync, lstatSync, openSync, } from "node:fs";
...
function exactHttpsIssuer(value) {
    ...
    if (url.protocol !== "https:" ||
        ...
        url.search ||
        url.hash) {
        throw new Error("External identity issuer must be an exact HTTPS URL");
    }
    return value;
}
...
export class SqliteLifecycleStore {
        ...
        return this.#transaction(() => {
          ...
          WHERE eim.issuer = ? AND eim.external_subject_id = ? AND eim.organization_key = ?
            ...
            if (toState === "export_only" || toState === "closed") {
                ...
                this.#run("INSERT INTO retention_transitions VALUES (?, ?, ?, ?, ?, ?)", this.#id("retention"), row.account_id, toState === "export_only"
                    ? "archive_window_started"
                    : "operational_data_closed", toState === "export_only"
                    ? new Date(Date.parse(timestamp) + 30 * 86_400_000).toISOString()
                    : timestamp, reasonCode, timestamp);
            }
    ...
    requestRecovery(recoveryCredential, proposedIdentityInput) {
        const proposedIdentity = this.#identity(proposedIdentityInput);
        const parsed = parseRecoveryCredential(recoveryCredential);
        const rows = this.#db
            .prepare(`SELECT rc.recovery_id, rc.account_id, rc.verifier_bytes,
        ...
        WHERE rc.state = 'active' AND eim.state = 'active' AND a.state != 'closed'
          AND (? = '' OR rc.recovery_id = ?)`)
            .all(parsed.recoveryId, parsed.recoveryId);
        let match;
        for (const row of rows) {
            if (verifyRecoverySecret(parsed.secret, row.verifier_bytes))
                match = row;
        }
        if (!match)
        ...
        try {
            this.#transaction(() => {
                const occupied = this.#db
                    .prepare(`SELECT 1 AS found FROM external_identity_mappings
            WHERE issuer = ? AND external_subject_id = ? AND organization_key = ?
              AND state = 'active'`)
                    .get(proposedIdentity.issuer, proposedIdentity.subject, proposedIdentity.organizationKey);
                if (occupied)
                    throw new Error("Recovery request is unavailable");
                this.#run(`INSERT INTO recovery_requests VALUES (?, ?, ?, ?, ?, ?, 'pending', ?, NULL, NULL)`, requestId, match.account_id, match.identity_mapping_id, proposedIdentity.issuer, proposedIdentity.subject, proposedIdentity.organizationKey, timestamp);
                this.#db
                    .prepare("UPDATE recovery_credentials SET last_used_at = ? WHERE recovery_id = ? AND state = 'active'")
    ...
    approveRecovery(recoveryRequestId, reviewerReference) {
        ...
        return this.#transaction(() => {
            const row = this.#db
                .prepare(`SELECT rr.*, a.mapping_revision, a.state AS account_state
          ...
          WHERE issuer = ? AND external_subject_id = ? AND organization_key = ?
            AND state = 'active'`)
                .get(row.proposed_issuer, row.proposed_external_subject_id, row.proposed_organization_key);
            if (occupied)
                throw new Error("Recovery approval is unavailable");
            this.#db
                .prepare("UPDATE external_identity_mappings SET state = 'replaced', replaced_at = ? WHERE identity_mapping_id = ? AND state = 'active'")
                .run(timestamp, row.current_identity_mapping_id);
            this.#run("INSERT INTO external_identity_mappings VALUES (?, ?, ?, ?, ?, 'active', ?, ?, NULL)", mappingId, row.proposed_issuer, row.proposed_external_subject_id, row.proposed_organization_key, row.account_id, revision, timestamp);
            this.#db
                .prepare("UPDATE accounts SET mapping_revision = ?, updated_at = ? WHERE account_id = ?")
                .run(revision, timestamp, row.account_id);
            this.#db
                .prepare("UPDATE recovery_credentials SET state = 'rotated', rotated_at = ? WHERE account_id = ? AND state = 'active'")
                .run(timestamp, row.account_id);
            this.#run("INSERT INTO recovery_credentials VALUES (?, ?, 'scrypt-v1', ?, 'active', ?, NULL, NULL)", recoveryId, row.account_id, verifier, timestamp);
            this.#db
                .prepare("UPDATE recovery_requests SET state = 'approved', reviewed_at = ?, reviewer_reference = ? WHERE recovery_request_id = ? AND state = 'pending'")
                .run(timestamp, reviewerReference, recoveryRequestId);
            this.#run("INSERT INTO account_state_events VALUES (?, ?, ?, ?, 'identity_recovery_approved', 'operator', ?, ?)", this.#id("event"), row.account_id, row.account_state, row.account_state, timestamp, revision);
            return Object.freeze({
                recoveryRequestId,
    ...
    recordArchive(identityInput, input) {
        ...
        return this.#transaction(() => {
            const row = this.#db
                .prepare(`SELECT a.account_id, tc.tenant_id, wc.workspace_id
          FROM external_identity_mappings eim
          JOIN accounts a ON a.account_id = eim.account_id
          JOIN tenant_catalog tc ON tc.owner_account_id = a.account_id
          ...
          WHERE eim.issuer = ? AND eim.external_subject_id = ? AND eim.organization_key = ?
            ...
            AND a.state IN ('active', 'reconsent_required', 'export_only')`)
                .get(identity.issuer, identity.subject, identity.organizationKey);
            if (!row)
                throw new Error("Archive operation is unavailable");
            this.#run("INSERT INTO archive_receipts VALUES (?, ?, ?, ?, 'llmthink-archive-v1', ?, ?, ?, ?)", receiptId, row.account_id, row.tenant_id, row.workspace_id, Buffer.from(input.contentSha256, "hex"), input.byteLength, input.itemCount, timestamp);
            return Object.freeze({
    ...
    archiveContext(identityInput) {
        const identity = this.#identity(identityInput);
        const row = this.#db
            .prepare(`SELECT a.subject_id, tc.tenant_id, wc.workspace_id
        FROM external_identity_mappings eim
        JOIN accounts a ON a.account_id = eim.account_id
        JOIN tenant_catalog tc ON tc.owner_account_id = a.account_id
        JOIN workspace_catalog wc ON wc.tenant_id = tc.tenant_id AND wc.kind = 'initial'
        ...
        return async (identity) => {
            ...
            WHERE eim.issuer = ? AND eim.external_subject_id = ? AND eim.organization_key = ?
              ...
              AND tc.state = 'active' AND wc.state = 'active' AND sp.state = 'active'`)
                ...
                if (!row)
                    throw new Error("External account mapping is unavailable");
    ...
    #migrate() {
        ...
        if (!exists) {
            ...
            this.#transaction(() => {
                ...
                this.#db
                    .prepare("INSERT INTO schema_metadata VALUES (1, ?, ?, '0002-recovery-export', ?)")
                    .run(SCHEMA_VERSION, timestamp, sha256(Buffer.from(MIGRATION_0002, "utf8")));
            });
            return;
        }
        const row = this.#db
        ...
        if (row?.schema_version === 1 &&
            ...
            this.#transaction(() => {
                this.#db.exec(MIGRATION_0002);
                this.#db
                    .prepare("UPDATE schema_metadata SET schema_version = 2, migrated_at = ?, migration_id = '0002-recovery-export', migration_sha256 = ? WHERE singleton = 1")
    ...
    #identity(identity) {
        return Object.freeze({
            ...
            subject: boundedIdentity(identity.subjectId, "external identity subject"),
            organizationKey: identity.organizationId === undefined
                ? ""
                : boundedIdentity(identity.organizationId, "external identity organization"),
        });
    }
    #existing(identity) {
        const row = this.#db
...
function parseRecoveryCredential(value) {
    const parts = value.split(".");
    if (parts[0] !== RECOVERY_PREFIX ||
...
function createRecoveryVerifier(secret, salt) {
    return Buffer.concat([Buffer.from(salt), scryptSync(secret, salt, 32)]);
}
function verifyRecoverySecret(secret, value) {
