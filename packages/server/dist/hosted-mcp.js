...
import { assertVerifiedRequestContext, InMemoryLlmthinkRateLimiter, LlmthinkSecurityBoundary, } from "./security.js";
...
function registerTools(server, application, context, textLimit, security) {
    ...
    server.registerTool(HOSTED_MCP_TOOL_NAMES.help, {
    ...
    }, ({ topic, tool, error_code, dsl_topic }) => run(HOSTED_MCP_TOOL_NAMES.help, async () => mcpHelp({
        topic,
        tool,
        errorCode: error_code,
        dslTopic: dsl_topic,
    })));
    server.registerTool(HOSTED_MCP_TOOL_NAMES.audit, {
        ...
        annotations: READ_ONLY,
    }, ({ text, document_id }) => run(HOSTED_MCP_TOOL_NAMES.audit, async () => ({
        ...(await application.audit({ text, documentId: document_id }, context)),
    })));
    server.registerTool(HOSTED_MCP_TOOL_NAMES.create, {
        ...
        annotations: WRITE,
    }, ({ thought_id, draft_text, idempotency_key, request_digest }) => run(HOSTED_MCP_TOOL_NAMES.create, async () => snapshot(await application.createThought({
    ...
    server.registerTool(HOSTED_MCP_TOOL_NAMES.get, {
        description: "Get one thought snapshot from the authenticated llmthink tenant/workspace.",
        inputSchema: thoughtIdShape,
        annotations: READ_ONLY,
    }, ({ thought_id }) => run(HOSTED_MCP_TOOL_NAMES.get, async () => snapshot(await application.getThought(ref(context, thought_id), context))));
    server.registerTool(HOSTED_MCP_TOOL_NAMES.list, {
        ...
        annotations: READ_ONLY,
    }, ({ cursor, limit, status }) => run(HOSTED_MCP_TOOL_NAMES.list, async () => {
    ...
    server.registerTool(HOSTED_MCP_TOOL_NAMES.search, {
        ...
        annotations: READ_ONLY,
    }, ({ query, cursor, limit, include_reflections }) => run(HOSTED_MCP_TOOL_NAMES.search, async () => {
    ...
    server.registerTool(HOSTED_MCP_TOOL_NAMES.finalize, {
    ...
    }, ({ thought_id, expected_revision, final_text, confirmation_token, idempotency_key, request_digest, }) => run(HOSTED_MCP_TOOL_NAMES.finalize, async () => snapshot(await application.finalizeThought({
    ...
    server.registerTool(HOSTED_MCP_TOOL_NAMES.reflect, {
        ...
        annotations: WRITE,
    }, ({ thought_id, expected_revision, kind, text, idempotency_key, request_digest, }) => run(HOSTED_MCP_TOOL_NAMES.reflect, async () => snapshot(await application.addReflection({
        ref: ref(context, thought_id),
        expectedRevision: expected_revision,
        kind,
        text,
    ...
    server.registerTool(HOSTED_MCP_TOOL_NAMES.delete, {
        ...
        annotations: CONSEQUENTIAL_WRITE,
    }, ({ thought_id, expected_revision, idempotency_key, request_digest }) => run(HOSTED_MCP_TOOL_NAMES.delete, async () => ({
...
function registerOnboardingTool(server, principal, issueUrl, textLimit) {
    server.registerTool(HOSTED_MCP_TOOL_NAMES.onboarding, {
        description: "Create a short-lived, single-use browser link to review and explicitly accept the current llmthink terms. This does not itself record agreement or create a tenant.",
        inputSchema: {},
        annotations: WRITE,
    }, async () => toolResult({
...
async function handleMcpRequest(request, response, options, requestLimit, textLimit, security, onboardingRateLimiter) {
    ...
    try {
        ...
        if (onboardingPrincipal && options.onboardingMcp) {
            registerOnboardingTool(server, onboardingPrincipal, options.onboardingMcp.issueUrl, textLimit);
        }
        else {
...
export function createLlmthinkHostedMcpHandler(options) {
    ...
    return async (request, response) => {
        ...
        await handleMcpRequest(request, response, options, requestLimit, textLimit, security, onboardingRateLimiter);
    };
}
