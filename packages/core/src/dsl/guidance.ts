...
import {
  ...
  type DslqlFunctionCategory,
} from "../dslql/functions.js";
...
function dslqlFunctionSignature(
  name: string,
...
function dslqlFunctionNames(category: DslqlFunctionCategory): string {
  return DSLQL_FUNCTION_SPECS.filter((entry) => entry.category === category)
    .map((entry) => `${entry.name}()`)
    .join("、");
}

const DSLQL_FUNCTION_DETAILS = DSLQL_FUNCTION_SPECS.map(
  (entry) =>
    `\`${dslqlFunctionSignature(entry.name, entry.operands)}\` -> ${entry.result}: ${entry.summary}`,
);
...
const HELP_NODES: HelpNode[] = [
  ...
  },
  {
    ...
    index: [
      ...
      },
      {
        ...
        label: "top-level",
        summary:
          "framework / domain / problem / step / confidence / declared_confidence / query の入口",
      },
      {
        key: "syntax.step",
        label: "step",
  ...
  },
  {
    ...
    summary:
      "evidence 本文を補足する匿名 resource と、その locator / metadata 契約。",
    quick: [
  ...
  },
  {
    ...
    summary:
      "annotation kind の閉じた集合と、problem / text-bearing statement への付与方法。",
    quick: [
  ...
  },
  {
    ...
    summary:
      "同一 problem / viewpoint scope で decision 間の相対比較を記述する。",
    quick: [
  ...
  },
  {
    ...
    summary:
      "入力端と明示 scoring edge の信頼度を、有理数区間と直交する認識タグで宣言する。",
    quick: [
  ...
  },
  {
    ...
    summary:
      "目的文や思考 profile から query テンプレートと代表例へ逆引きする入口。",
    quick: [
    ...
    index: [
      {
        ...
        label: "ideation",
        summary:
          "発散 -> 収束 -> クラスタ化 -> ラベル -> 結論を既存 role で記述したい",
      },
      {
  ...
  },
  {
    ...
    summary:
      "その他ユースケースは syntax 追加ではなく representative profile を段階追加する。",
    quick: [
...
function helpInvocationExamples(
  node: HelpNode,
  request: DslHelpRequest,
): string[] {
...
const PARSE_ERROR_HELP_RULES: ParseErrorHelpRule[] = [
  ...
  },
  {
    ...
    help: {
      rationale:
        "problem は header 行の次に quoted text または `|` marker 付き block text を持つ。",
      expectedSyntax: [
        "problem P1:",
        '  "監査したい問題文"',
        "problem P2:",
  ...
  },
  {
    ...
    help: {
      rationale:
        "premise は 'premise Id:' の次に quoted text または block text を持つ。",
      expectedSyntax: [
        "step S1:",
        "  premise PR1:",
        '    "現在の前提"',
  ...
  },
  {
    ...
    help: {
      rationale:
        "evidence は 'evidence Id:' の次に必須の quoted text または block text を持ち、その後に任意の resource / annotation を置ける。",
      expectedSyntax: [
        "step S1:",
        "  evidence EV1:",
        '    "観測事実"',
  ...
  },
  {
    ...
    help: {
      rationale:
        "resource は匿名 block とし、url / file / blob の locator をちょうど 1 つ、任意の digest / mime / label を各 1 回まで持つ。",
      expectedSyntax: [
        "evidence EV1:",
        '  "観測事実"',
        "  resource:",
  ...
  },
  {
    ...
    help: {
      rationale:
        "pending は 'pending Id:' の次に quoted text または block text を持つ。",
      expectedSyntax: [
        "step S1:",
        "  pending PD1:",
        '    "未確定事項"',
  ...
  },
  {
    ...
    help: {
      rationale:
        "query は 'query Id:' の次に expression を持つ。DSLQL の詳細は query help を辿る。",
      expectedSyntax: [
        "query Q1:",
        "  .document.problems[] | select(.id == @P1) | related_decisions()",
      ].join("\n"),
