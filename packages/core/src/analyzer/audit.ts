...
function collectBasedOnTargetIds(document: DocumentAst): Set<string> {
  return new Set(
    createDocumentDeclarationIndex(document)
      .declarations.filter(
        (declaration) =>
          declaration.kind === "problem" || declaration.kind === "statement",
      )
      .map((declaration) => declaration.id),
  );
}

function collectDslqlDeclaredIds(document: DocumentAst): Set<string> {
...
function collectTextLintTargets(document: DocumentAst): TextLintTarget[] {
  ...
  for (const step of document.steps) {
    ...
    targets.push(
      ...step.statement.annotations.map((annotation) => ({
...
function addTextBodyLintIssues(
  issues: AuditIssue[],
  document: DocumentAst,
): void {
  for (const target of collectTextLintTargets(document)) {
    if (
      target.body.syntax === "quoted" &&
...
interface EvaluatedQueryValues {
  values: DslqlValue[];
  error?: DslqlEvaluationError;
}

function evaluateQueryValues(
  expression: string,
  runtime: DslqlRuntime | undefined,
): EvaluatedQueryValues {
...
async function buildQueryResults(
  ...
  return document.queries.map((query, queryIndex) => {
    ...
    const evaluated = evaluateQueryValues(
      query.expression,
...
interface SemanticAnalysisResult {
  ...
  metadata: AuditSemanticAnalysis;
}

function requestedSemanticProvider(options?: AuditOptions): string {
  if (options?.semanticEmbedder) return "custom";
  return (
    options?.embeddings?.provider ??
    process.env.LLMTHINK_EMBEDDING_PROVIDER ??
    "ollama"
  );
}

async function createSemanticAnalysis(
  decisions: Array<{ stepId: string; statement: DecisionStatement }>,
  options?: AuditOptions,
