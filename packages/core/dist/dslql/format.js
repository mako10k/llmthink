...
function formatPathSegment(segment) {
    ...
    if (segment.kind === "iterate")
        return `[]${optional}`;
    if (segment.kind === "index")
        return `[${segment.index}]${optional}`;
...
function needsSamePrecedenceParentheses(expression, parent, side) {
...
function formatChild(expression, parent, side) {
    const formatted = formatExpression(expression);
    if (precedence(expression) < precedence(parent) ||
...
function formatBinary(expression) {
    return `${formatChild(expression.left, expression, "left")} ${expression.operator} ${formatChild(expression.right, expression, "right")}`;
}
    ...
    switch (expression.kind) {
        ...
        case "object":
            return `{${expression.fields
                .map((field) => `${objectKey(field.key)}: ${formatExpression(field.value)}`)
                .join(", ")}}`;
        case "call":
...
function formatPath(origin, segments) {
